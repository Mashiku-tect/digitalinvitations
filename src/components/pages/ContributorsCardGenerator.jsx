...
const CardDesigner = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const response = await api.get('/api/events/contributionevents', {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        setEvents(response.data.events || []);
        //console.log('all events are',response.data.events)
      } catch (error) {
        const errrormessage = error.response?.data?.message || 'Failed to load events';
  ...
  const handleEventSelect = async (e) => {
    ...
    try {
      ...
      const errormessage = error.response?.data?.message || 'Failed to load event guests';
  ...
  const publishCards = async () => {
    if (!selectedEvent || !guests.length || !image) {
      toast.error("Please select an event, upload an image, and ensure guests are loaded");
      return;
    }
    ...
    try {
      ...
      const errormessage = error.response?.data?.message || 'Failed to publish cards';
      toast.error(errormessage);
    } finally {
      setPublishing(false);
    }
  ...
  const downloadAllInvitations = async () => {
    if (!guests.length || !image) {
      toast.error("Please upload an image and select an event with guests");
      return;
    }
    ...
    <Layout>
      <div className="min-h-screen bg-gray-100 p-6 flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          <h1 className="text-3xl font-extrabold text-gray-800">🎉 Contribution Card Designing</h1>
          ...
          {/* Customization Panel - Only for Name */}
          {selectedId === "name" && (
            <div className="bg-white p-4 rounded-xl shadow space-y-4">
              <h2 className="text-lg font-semibold text-gray-700">Edit Name Styling</h2>
              <div>
